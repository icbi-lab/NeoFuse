...
def tmp_out_pep(xenoInFile, tmpOutFile1, tmpOutFile2, outDir, fGenes, peptidesFile, hla, ftype, confidence, stopCod, cores, pepList):
	gene_file=[]
	...
	with open(tmpOutFile1, "+w") as out_file:
		for i in range(0, len(fGenes)):
			...
			if counter <= cores:
				out_file.write('''netMHCpan -a %s -p %s -BA > %s &\n''' % (hla, pepFile, out))
			else:
				out_file.write('''netMHCpan -a %s -p %s -BA > %s &\n''' % (hla, pepFile, out))
				out_file.write("wait\n")
				counter = 1
...
if __name__ == "__main__":
	# Parse arguments
	parser = argparse.ArgumentParser(usage='association_netMHCpan.py [-h] -x {/path/to/xeno/input/} -l {path/to/OptiType/input} -o {/path/to/output/} -c (NCores)')
	...
	tmp_out_pep(xenoFile, outFile1, outFile2, outDir, fGene, peps, hla, ftype, confidence, stopCod, cores, pepList)
