...
while read -r var path
do
	if [ "$var" == "OptiRef" ]; then
		OPTIREF=$path
	elif [ "$var" == "Optitype" ]; then
		Optitype=$path
	elif [ "$var" == "arriba" ]; then
		arriba=$path
	fi
done < ../config.txt
OUTDIR="./"
...
while getopts "i:o::m::M::n::t::T::c::s::g::a::N::" opt;
do
	case $opt in
	i)	IN="$OPTARG";;
	o)  OUTDIR="$OPTARG";;
...
BLACKLIST=${DIR}/adb/blacklist_hg38_GRCh38_2018-11-04.tsv.gz

# process args
if test -z "$IN" 
then
	echo "usage: NeoFuse_multi -i <input> -o [output] -m [peptide min length] -M [peptide Max length] -n [cores] -c [confidence level] -t [affinity threshold] -T [rank threshold] -s <STAR index> -g <FASTA file> -a <GTF file>"
...
do
	...
	if test -f "$READ2"; then
		...
		if [ `echo $?` != 0 ]; then
			echo "An error occured during STAR/Arriba run, check the log files in $LOGSDIR for more details"
			exit 1
		else
			:
		fi
		...
		if [ `echo $?` != 0 ]; then
			echo "An error occured during OptiType run, check $LOGSDIR$FILE.optitype.log for more details"
			exit 1
		else
			:
		fi
		## Save output and remove temporary files
		OutFile=${OUTDIROPTI}"/"$FILE"_HLA_Optitype.txt"
		...
		if [ `echo $?` != 0 ]; then
			echo "An error occured during featureCounts run, check $LOGSDIR$FILE.featureCounts.log for more details"
			exit 1
		else
			:
		fi
	...
	Rscript ${DIR}/counts_to_tpm.R \
		$OUTDIRCOUNTS$FILE.counts.txt \
		$OUTDIRTPM$FILE.tpm.txt \
		$OUTDIRRPKM$FILE.rpkm.txt > $LOGSDIR$FILE.counts_to_tpm.log 2>&1

	if [ `echo $?` != 0 ]; then
		echo "An error occured during conversion of Raw Counts to TPM and FPKM, check $LOGSDIR$FILE.counts_to_tpm.log for more details"
		exit 1
	else
		:
	fi
	...
	python3 ${DIR}/cleave_peptides.py -i $OUTDIRARRIBA$FILE.fusions.tsv -o $OUTDIRCLEAVEPEP$FILE -p $PEPLEN > $LOGSDIR$FILE.cleave.log 2>&1
	if [ `echo $?` != 0 ]; then
		echo "An error occured while searching for peptides, check $LOGSDIR$FILE.cleave_peptides.log for more details"
		exit 1
	else
		:
	fi
	...
	if [ "$NETMHCPAN" == "false" ]; then
		...
		for filename in $OUTDIRCLEAVEPEP*.tsv; do
			fou=$(echo ${filename##*/} | sed s/_xeno// | sed s/\.tsv//)
   			python3 ${DIR}/association.py -x $filename -l ${OUTDIROPTI}"/"$FILE"_HLA_Optitype.txt" -o $FINALTMP$fou -c $CORES > $LOGSDIR$FILE.association.log 2>&1
		done
		if [ `echo $?` != 0 ]; then
			echo "An error occured while creating the assosciation files, check $LOGSDIR$FILE.association.log for more details"
			exit 1
		else
			:
		fi
		...
		source /usr/local/bioinf/conda/bin/activate mhcflurry
		for i in $FINALTMP*_TEST_OUT.sh; do
			name=$(echo ${i##*/} | sed s/_TEST_OUT.sh//)
			sh $i > $LOGSDIR$name"_MHCFlurry.log" 2>&1
			if [ `echo $?` != 0 ]; then
				echo "An error occured during the MHCFlurry run, check $LOGSDIR$name\"_MHCFlurry.log\" for more details"
				exit 1
			else
				:
			fi
		...
		wait
		source /usr/local/bioinf/conda/bin/deactivate
		sleep 30 # Extra time to release resources
		
		echo " Creating Final Ouptut:" `date +"%T"` | sed "s/^/[NeoFuse] /"
		for j in $PEPLEN; do
			python3 ${DIR}/build_temp.py -a $FINALTMP*$j"_ASSOCIATIONS_OUT.txt" -o $FINALTMP$FILE"_"$j > $LOGSDIR$FILE.final.log 2>&1
			if [ `echo $?` != 0 ]; then
				echo "An error occured while creating the final output files, check $LOGSDIR$FILE.final.log for more details"
				exit 1
			else
				:
			fi
		...
		for filename in $OUTDIRCLEAVEPEP*.tsv; do
			fou=$(echo ${filename##*/} | sed s/_xeno// | sed s/\.tsv//)
   			python3 ${DIR}/association_netMHCpan.py -x $filename -l ${OUTDIROPTI}"/"$FILE"_HLA_Optitype.txt" -o $FINALTMP$fou -c $CORES > $LOGSDIR$FILE.association.log 2>&1
		done
		if [ `echo $?` != 0 ]; then
			echo "An error occured while creating the assosciation files, check $LOGSDIR$FILE.association.log for more details"
			exit 1
		...
		for i in $FINALTMP*_TEST_OUT.sh; do
			name=$(echo ${i##*/} | sed s/_TEST_OUT.sh//)
 			...
 			if [ `echo $?` != 0 ]; then
 				echo "An error occured during the netMHCpan run, check $LOGSDIR$name\"_netMHCpan.log\" for more details"
 				exit 1
	 		else
	 			:
	 		fi
		...
		echo " Creating Final Ouptut:" `date +"%T"` | sed "s/^/[NeoFuse] /"
		for j in $PEPLEN; do
			python3 ${DIR}/build_temp_netMHCpan.py -a $FINALTMP*$j"_ASSOCIATIONS_OUT.txt" -o $FINALTMP$FILE"_"$j > $LOGSDIR$FILE.final.log 2>&1
			if [ `echo $?` != 0 ]; then
				echo "An error occured while creating the final output files, check $LOGSDIR$FILE.final.log for more details"
				exit 1
			else
	...
	python3 ${DIR}/filter.py -i $FINALOUTDIR$FILE"_unfiltered.tsv" -o $FINALOUTDIR$FILE"_tmp_filtered.tsv" -t $THRESHOLD -T $RANK -c $CONFIDENCE >> $LOGSDIR$FILE.final.log 2>&1
	if [ `echo $?` != 0 ]; then
		echo "An error occured while filtering the final output files, check $LOGSDIR$FILE.final.log for more details"
		exit 1
	else
		:
	fi
	...
	python3 ${DIR}/final_out.py -t $OUTDIRTPM$FILE.tpm.txt -i $FINALOUTDIR$FILE"_tmp_filtered.tsv" -o $FINALOUTDIR$FILE"_filtered.tsv" >> $LOGSDIR$FILE.final.log 2>&1
	if [ `echo $?` != 0 ]; then
		echo "An error occured while appending TPM values to the final output files, check $LOGSDIR$FILE.final.log for more details"
		exit 1
	else
		:
	fi
