...
while getopts "1:2::d::o::m::M::n::t::T::c::s::g::a::N::" opt;
do
	case $opt in
	1)	READ1="$OPTARG";;
	2)	READ2="$OPTARG";;
...
if test -f "$READ2"; then
	...
	wait
	if [ `echo $?` != 0 ]; then
		echo "An error occured during RazerS3 run, check $LOGSDIR$FILE.STAR.err for more details"
	else
		:
	fi
...
echo " All the results and log files can be found in $OUTDIR" | sed "s/^/[NeoFuse] /"
echo "[-------------------------------- [NeoFuse] --------------------------------]"
echo
