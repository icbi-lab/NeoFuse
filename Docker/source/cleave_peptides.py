...
def sequence_hound(
    inFile, fusionGene, bkpoint1, bkpoint2, pepSeq, ftype, confidence, stopCod
):
    drows = []
...
def window_onslaught(
    outFile, pepSeq, fGene, bkpoint1, bkpoint2, winSize, ftype, confidence, stopCod
):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        usage="cleave_peptides.py [-h] -i {/path/to/input/} -o {/path/to/input/} -p {8,9,10,11}",
    ...
    sequence_hound(
        inFile, fusionGene, bkpoint1, bkpoint2, pep_seq, ftype, confidence, stopCod
    )
    ...
    for a in pepLen:
        ...
        out += "_xeno_%s.tsv" % a
        window_onslaught(
            out, pep_seq, fusionGene, bkpoint1, bkpoint2, a, ftype, confidence, stopCod
        )
