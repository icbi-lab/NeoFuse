...
def final_out(infileAssoc, outfile):
    ...
    with open(outfile, "+w") as out_file:
        out_file.write(
            "Fusion\tGene1\tGene2\tBreakpoint1\tBreakpoint2\tSplit_Reads1\tSplit_Reads2\tDiscordant_Reads\tClosest_Breakpoint1\tClosest_Breakpoint2\tHLA_Type\tFusion_Peptide\tnetMHCpan_IC50\tnetMHCpan_Rank\tEvent_Type\tStop_Codon\tConfidence\n"
        )
        for file in files:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        usage="build_temp_netMHCpan.py [-h] -a {path/to/Associations.txt/input} -o {/path/to/output/}"
    )
    parser.add_argument(
