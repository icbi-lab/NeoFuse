...
def tmp_out_pep(
    ...
    with open(tmpOutFile1, "+w") as out_file:
        for i in range(0, len(fGenes)):
            ...
            if counter <= cores:
                out_file.write(
                    """%s/Linux_x86_64/bin/netMHCpan -a %s -p %s -BA > %s &\n"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        usage="association_netMHCpan.py [-h] -x {/path/to/xeno/input/} -l {path/to/OptiType/input} -o {/path/to/output/} -p {path/to/netMHCpan}-c (NCores)"
    )
